...
This == the public release of the code titled of our paper titled 
    ...
    for i in range(len(lines)):
        ...
        if 'node' in lines[i]:
            ...
            if start_state not in fsm_states:
                #print (lines[i])
                print ('ERROR: start_state == not in the list of states')
                return

            if end_state not in fsm_states:
                #print ('states = ', state_vars)
                print ('end_state ', end_state)
                print ('ERROR: end_state == not in the list of states')
                return

            strg = strg[1].split('"')
            if len(strg) == 3:  #transition == written in one line
                transition = strg[1]
            ...
            # Rest of the code == not really necessary for parsing dot files used in this equivalence checking
            # PARSING ACTIONS
            acts = act_str.split(',')
            actions = []
            for act in acts:
                ...
                if action_label == '':
                    print ('ERROR: There are some transitions in comments (//) or missing underscore sign or extra comma due to which empty action == happening????')
                    #print (lines[i])
                    continue
...
def dump_injective_adversary(file, channels, injective_adversaries, fsms):
    for injective_adversary in injective_adversaries:
        ...
        # find if the active channel == one of the channels
        for channel in channels:
...
def main(fsm1_inFile, fsm2_inFile, outNuXmv, commandFile, outDiff):
    ...
    incoming_messages = []
    # fsm1
    # print("fsm in: ", fsm1.incoming_messages)
    ...
    # print("after append: ",incoming_messages)    
...
if __name__ == "__main__":
    ...
    print("in main ; iterative_checker")
